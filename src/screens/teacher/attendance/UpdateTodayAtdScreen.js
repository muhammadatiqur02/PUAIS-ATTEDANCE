...
const UpdateTodayAtdScreen = ({ navigation, route }) => {
      ...
      await axios.get(url).then((res) => {
        if (res.data.MessageCode == 200) {
          let arr = res.data.Data.map((item) => {
            if (item.status === 'Absent') item.isSelected = true;
  ...
  const selectionHandelr = async (ind, type) => {
    let arr = students.map((item, index) => {
      if (ind === index) {
        if (type === 'select') {
  ...
  const saveAttendance = async () => {
    try {
      ...
      ids = ids.slice(0, -1);  // Remove the trailing comma
      ...
      await axios.post(url).then((res) => {
        if (res.data.MessageCode === 200) {
          const msg = `Total Students: ${students.length}\nPresent Students: ${students.length - count}\nAbsent Students: ${count}`;
          setMessage(msg);
    ...
    <>
      ...
      <View style={{ alignItems: 'center' }}>
        <Text style={{ fontSize: 18, fontWeight: 'bold', color: colors.black }}>
          {course.semester} {course.coursecode}-{course.coursename}{course.sectionname}
        </Text>
      ...
      <ScrollView
        ...
        style={{ padding: 10 }}
        refreshControl={<RefreshControl refreshing={loading} onRefresh={getStudent} />}
      >
        {students?.sort((a, b) => a.roll - b.roll).map((student, index) => {
          return (
            <View key={index} style={[styles.card, { backgroundColor: student.isSelected ? colors.danger_light : colors.success_light }]}>
              ...
              <View style={{ flex: 1 }}>
                {student.isSelected
                  ? <AppFilledButton danger title={'Absent'} onPress={() => selectionHandelr(index, '')} />
